...
#precache( "triggerstring", "ZOMBIE_TRAP_ACTIVE" );
#precache( "triggerstring", "ZOMBIE_TRAP_COOLDOWN" );

REGISTER_SYSTEM_EX( "zm_traps", &__init__, &__main__, undefined )
...
//-------------------------------------------------------------------------------
// Trigger Visibility: 
// Death machine powerup set .is_drinking on the player.
//-------------------------------------------------------------------------------
function private update_trigger_visibility() //self = trigger
{
	self endon( "death" );
	
	//similar to the logic in _zm_perks.gsc to update prompt visibility
	const N_DIST_SQ = 16384; // 128 * 128
	
	while( true )
	{
		for( i = 0; i < level.players.size; i++ )
		{
			if( DistanceSquared( level.players[i].origin, self.origin ) < N_DIST_SQ )
			{
				if( IS_DRINKING( level.players[i].is_drinking ) )
				{
					self SetInvisibleToPlayer( level.players[i], true );
				}
				else
				{
					self SetInvisibleToPlayer( level.players[i], false );
				}
			}
		}
		wait 0.25;
