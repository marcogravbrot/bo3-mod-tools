...
#precache( "string", "KILLSTREAK_REMOTE_MORTAR_NOT_USABLE" );
#precache( "string", "KILLSTREAK_REMOTE_MORTAR_INBOUND" );
#precache( "string", "KILLSTREAK_REMOTE_MORTAR_HACKED" );
...
function init()
{
	...
	killstreaks::register_strings( "remote_mortar", &"KILLSTREAK_EARNED_REMOTE_MORTAR", &"KILLSTREAK_REMOTE_MORTAR_NOT_AVAILABLE", &"KILLSTREAK_REMOTE_MORTAR_INBOUND", &"KILLSTREAK_REMOTE_MORTAR_HACKED" );
...
function remote_mortar_killstreak( hardpointType )
{
	...
	self killstreaks::set_killstreak_delay_killcam( hardpointType );
	...
	self killstreaks::play_killstreak_start_dialog( "remote_mortar", self.pers["team"], killstreak_id );
	...
	self clientfield::set( CLIENT_FIELD_OPERATING_REAPER, 0 );
...
{
	self clientfield::set( CLIENT_FIELD_OPERATING_REAPER, 1 );
	...
	remote clientfield::set( "enemyvehicle", ENEMY_VEHICLE_ACTIVE );
	remote.attackers = [];
	remote.attackerData = [];
...
function remote_killstreak_end( explode, disconnected )
{
	...
	if ( isdefined( self.owner ) )
	{
		...
		self.owner Unlink();
		self.owner killstreaks::reset_killstreak_delay_killcam();
...
function player_fire_think( remote )
{
	...
	end_time = GetTime() + ( GetDvarFloat( "scr_remote_mortar_lifetime" ) * 1000 );
