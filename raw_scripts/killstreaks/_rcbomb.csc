...
REGISTER_SYSTEM( "rcbomb", &__init__, undefined )
...
function __init__()
{
	...
	// vehicle flags	
	clientfield::register( "vehicle", "rcbomb_stunned", VERSION_SHIP, 1, "int", &callback::callback_stunned, !CF_HOST_ONLY, !CF_CALLBACK_ZERO_ON_NEW_ENT );
	
	vehicle::add_vehicletype_callback( RCBOMB_VEHICLE,&spawned );
}
...
function boost_blur( localClientNum )
{
	...
	{
		EnableSpeedBlur( localClientNum, RCBOMB_BLUR_AMOUNT, RCBOMB_BLUR_INNER_RADIUS, RCBOMB_BLUR_OUTER_RADIUS, false, 0 );
		
		wait( RCBOMB_BLUR_BOOST_DURATION );
