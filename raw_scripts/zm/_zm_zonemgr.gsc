...
REGISTER_SYSTEM( "zm_zonemgr", &__init__, undefined )
	...
	if ( IsDefined( zone.volumes[0].target ) )
	{
			...
			foreach ( token in tokens )
			{
				switch( token )
				{
					...
					case "custom_spawner_entry":
						ARRAY_ADD( zone.a_loc_types[ "zombie_location" ], spot );
						break;
					default:
						...
						ARRAY_ADD( zone.a_loc_types[ token ], spot );
				}
			}
		...
		for( i = 0; i < barricades.size; i++ )
		{
			...
			for( j = 0; j < targets.size; j++ )
			{
				...
				{
					ARRAY_ADD( zone.zbarriers, targets[j] );
				}
			}
		}
		...
		for( i = 0; i < box_locs.size; i++ )
		{
			...
			{
				ARRAY_ADD( zone.magic_boxes, box_locs[i] );
			}
		}
	}	
}
...
{
	DEFAULT(level.zone_flags_updating,0); 
...
{
	DEFAULT(level.zone_flag_wait_throttle,0);
	...
	for( z=0; z<zkeys.size; z++ )
	{
		...
		if ( zone.is_enabled && zone.is_active && zone.is_spawning_allowed )
		{
			foreach( a_locs in zone.a_loc_types )
			{
				...
				{
					if ( isdefined( loc.is_enabled ) && loc.is_enabled == false )
					{
						continue;
					}
					...
					foreach ( token in tokens )
					{
						switch( token )
						{
							...
							case "faller_location":
							case "custom_spawner_entry":
								array::add( level.zm_loc_types[ "zombie_location" ], loc, false );
								break;
							default:
								...
								array::add( level.zm_loc_types[ token ], loc, false );
						}
					}
				}
			}
...
// Get an array of the active zone entities
...
function _debug_zones()
{
	...
	for ( i = 0; i < zkeys.size; i++ )
	{
		zoneName = zkeys[i];
		zone = level.zones[zoneName];
		_debug_show_zone( zone, ( RandomFloatRange( 0, 1.0 ), RandomFloatRange( 0, 1.0 ), RandomFloatRange( 0, 1.0 ) ), 0.2 );
