...
#precache( "string", "KILLSTREAK_MISSILE_DRONE_HACKED" );
...
function init()
{	
	clientfield::register( "toplayer", "missile_drone_active", VERSION_SHIP, 2, "int" );
	clientfield::register( "missile", "missile_drone_projectile_active", VERSION_SHIP, 1, "int" );
	clientfield::register( "missile", "missile_drone_projectile_animate", VERSION_SHIP, 1, "int" );
...
function missileDroneWatcher( hardpointType )
{
	...
	self clientfield::set_to_player( "missile_drone_active", MISSILE_DRONE_TARGET_DISABLED );
...
function updateTargetting()
{
	...
	for( ;; )
	{
		...
		if ( isdefined( self.droneTarget ) )
		{
			...
			self clientfield::set_to_player("missile_drone_active", MISSILE_DRONE_TARGET_ACTIVE );
		}
		else
		{
			self clientfield::set_to_player( "missile_drone_active", MISSILE_DRONE_TARGET_ENABLED );
		}
