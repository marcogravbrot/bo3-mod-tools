...
#precache( "string", "KILLSTREAK_STRAFERUN_HACKED" );
...
function watchdamage()
{
	...
	self.maxhealth = STRAFERUN_MAX_HEALTH;
	...
	for ( ;; )
	{
		...
		if ( damage_taken >= STRAFERUN_MAX_HEALTH )
		{
			self thread explode();
...
function watchForOwnerExit( owner )
{
	...
	owner util::waittill_any( "disconnect", "joined_team", "joined_spectators");
	self.leaveNextTime = true;
}
