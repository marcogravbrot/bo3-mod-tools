...
REGISTER_SYSTEM_EX( "zm_equipment", &__init__, &__main__, undefined )
...
function __init__()
{
	...
	{
		clientfield::register( "scriptmover", "equipment_activated", VERSION_SHIP, 4, "int" );
	}
}
...
function equipment_spawn_think()
{
	for ( ;; )
	{
		...
		if ( player zm_utility::in_revive_trigger() || IS_DRINKING(player.is_drinking) )
		{
			wait( 0.1 );
			continue;
		}
...
function setup_client_hintelem( ypos = 220, font_scale = 1.25 )
{
	...
	if( self IsSplitScreen() ) 
	{
		...
		{
			// 3 & 4 player : smaller screen - normal aspect ratio
			self.hintelem init_hint_hudelem( 160, 90, "center", "middle", font_scale * 0.8, 1.0 );
		}
		else
		{
			// 2 player : short wide screen
			self.hintelem init_hint_hudelem( 160, 90, "center", "middle", font_scale, 1.0 );
		}
	}
	else
	{
		self.hintelem init_hint_hudelem(320, ypos, "center", "bottom", font_scale, 1.0);
	}
}
