...
#precache( "locationselector", PLANE_MORTAR_LOCATION_SELECTOR );
#precache( "locationselector", PLANE_MORTAR_LOCATION_SELECTOR_DONE );
...
function init()
{
	...
	clientfield::register( "scriptmover", "planemortar_contrail", VERSION_SHIP, 1, "int" );
...
{
	self beginLocationMortarSelection( PLANE_MORTAR_LOCATION_SELECTOR, PLANE_MORTAR_SELECTION_RADIUS, PLANE_MORTAR_LOCATION_SELECTOR_DONE );
...
function doBombRun( position, yaw, team )
{
	...
	plane clientfield::set( "enemyvehicle", ENEMY_VEHICLE_ACTIVE );
	...
	plane.killcamEnt util::deleteAfterTime( PLANE_MORTAR_FLIGHT_TIME * 3 );
...
function followBomb( plane, position, direction, impact, player )
{
	...
	wait ( PLANE_MORTAR_FLIGHT_TIME * 5 / 12 );
...
function dropBomb( plane, bombPosition )
{
	...
	{
		if( plane.owner util::IsEnemyPlayer(target) && Distance2DSquared( target.origin, bombPosition ) < PLANE_MORTAR_AIM_ASSIST_RADIUS_SQ )
		{
