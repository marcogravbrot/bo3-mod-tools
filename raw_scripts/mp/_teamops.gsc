...
#insert scripts\mp\_teamops.gsh;
...
function getTeamopsTableID()
{
	...
	teamopsInfoTableID = TableLookupFindCoreAsset( GAMEOPS_EVENT_TABLE_NAME );
		
	if ( isdefined( teamopsInfoTableID ) )
	{
		teamopsInfoTableLoaded = true;
	}
	assert( teamopsInfoTableLoaded, "Team Ops Event Table is not loaded: " + GAMEOPS_EVENT_TABLE_NAME );
...
function init()
{
	...
	for( row = 1; row < GAMEOPS_EVENT_MAX_COUNT; row++ )
	{
		name = tableLookupColumnForRow( tableID, row, GAMEOPS_NAME );
		if( name != "" )
		{
			game["teamops"].data[name] = SpawnStruct();
			game["teamops"].data[name].description = tableLookupColumnForRow( tableID, row, GAMEOPS_DESCRIPTION );
			game["teamops"].data[name].pushevent = tableLookupColumnForRow( tableID, row, GAMEOPS_PUSH_EVENT );
			game["teamops"].data[name].popevent = tableLookupColumnForRow( tableID, row, GAMEOPS_POP_EVENT );
			game["teamops"].data[name].resetevent = tableLookupColumnForRow( tableID, row, GAMEOPS_RESET_EVENT );
			game["teamops"].data[name].count = int( tableLookupColumnForRow( tableID, row, GAMEOPS_COUNT ) );
			game["teamops"].data[name].time = int( tableLookupColumnForRow( tableID, row, GAMEOPS_TIME ) );
			game["teamops"].data[name].modes = StrTok( tableLookupColumnForRow( tableID, row, GAMEOPS_MODE ), "," );
			game["teamops"].data[name].rewards = StrTok( tableLookupColumnForRow( tableID, row, GAMEOPS_REWARD ), "," );
		}
	}
...
function getID( name )
{
	...
	for( row = 1; row < GAMEOPS_EVENT_MAX_COUNT; row++ )
	{
		_name = tableLookupColumnForRow( tableID, row, GAMEOPS_NAME );
		if( name == _name )
			return row;
	}
	return 0;
