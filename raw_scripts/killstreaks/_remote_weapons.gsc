...
function WatchRemoveRemoteControlledWeapon()
{
	...
	{
		WAIT_SERVER_FRAME; // Wait until any death animations and any other waits are finished
	}
...
{
	player = self;
	if( player useButtonPressed() && !player.throwingGrenade && !player meleeButtonPressed() && !player util::isUsingRemote() && !( isdefined( player.carryObject ) && IS_TRUE( player.carryObject.disallowRemoteControl ) ) )
	{
		return true;
	}
	else 
